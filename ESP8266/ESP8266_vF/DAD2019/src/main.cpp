...
const char* password = "javi_viera8";
const char* mqtt_server = "192.168.43.22";
const char* http_server = "192.168.43.22";
...
void makeGetRequestElemento(const char* nombreElemento){
    HTTPClient http;
...
void makePutRequestOperacion(const char* nombreElemento){
    HTTPClient http;
    // Abrimos la conexiÃ³n con el servidor REST y definimos la URL del recurso
    String url = "http://";
    url += http_server;
    ...
    root["nombre_usuario"] = "jaime";
    root["nombre_elemento"] = nombreElemento;
    String json_string;
...
void loop() {
  ...
  if (now - lastMsgRest > 2000) {
    if(espserial.available()>0){
          ...
          lastMsgRest = now;
          makeGetRequestElemento(elementos[dato]);
          makePutRequestOperacion(elementos[dato]);
          }

...
public class MqttExample extends AbstractVerticle {
	...
	public void start(Future<Void> startFuture) {
		...
		/*
		 * Nos conectamos al servidor que estÃ¡ desplegado por el puerto 1883 en la
		 * propia mÃ¡quina. Recordad que localhost debe ser sustituido por la IP de
		 * vuestro servidor. Esta IP puede cambiar cuando os desconectÃ¡is de la red, por
		 * lo que aseguraros siempre antes de lanzar el cliente que la IP es correcta.
		 */
		mqttClient.connect(1883, "localhost", s -> {

			/*
			 * Nos suscribimos al topic_2. AquÃ­ debera indicar el nombre del topic al que os
			 * querÃ©is suscribir. AdemÃ¡s, podÃ©is indicar el QoS, en este caso AT_LEAST_ONCE
			...
			mqttClient.subscribe("topic_2", MqttQoS.AT_LEAST_ONCE.value(), handler -> {
				if (handler.succeeded()) {
					 ...
					 * proceso serÃ­a el siguiente: El cliente exterbi envÃ­a un mensaje al servidor
					 * -> el servidor lo recibe y busca los clientes suscritos al topic -> el
					 * servidor reenvÃ­a el mensaje a esos clientes -> los clientes (en este caso el
			...
			/*
			 * Este timer envÃ­a mensajes desde el cliente al servidor cada 3 segundos.
			 */

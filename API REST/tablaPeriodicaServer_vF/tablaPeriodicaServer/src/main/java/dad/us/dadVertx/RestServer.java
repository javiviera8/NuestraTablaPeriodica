...
public class RestServer extends AbstractVerticle {
	...
	private void handlerInsertOperacion(RoutingContext routingContext) {
		...
		mySQLClient.getConnection(connection -> {
			if (connection.succeeded()) {
				connection.result().query("INSERT INTO operacion(nombre_tabla_periodica,nombre_usuario,nombre_elemento) VALUES ('" + body.getString("nombre_tabla_periodica") 
				+ "','" +  body.getString("nombre_usuario") + "','" + body.getString("nombre_elemento") + "')", result -> {
					if(result.succeeded()) {
						System.out.println("INSERT realizado con éxito en la base de datos.");
					}else {
						System.out.println("INSERT no se pudo realizar con éxtio");
